...
async function createClient() {
    const wallet = await DirectSecp256k1HdWallet.fromMnemonic(process.env.FAUCET_MNEMONIC, { prefix: process.env.PREFIX });
...
app.post('/credit', async (req, res) => {
    ...
    const amount = {
        denom,
        amount: `${Number(process.env.NUMBER_OF_TOKENS)*10**Number(process.env.DENOM_EXPONENT)}`,
    };
    ...
    try {
        ...
        const result = await client.sendTokens(firstAccount.address, address, [amount], fee, 'Credited by DTEAM');
